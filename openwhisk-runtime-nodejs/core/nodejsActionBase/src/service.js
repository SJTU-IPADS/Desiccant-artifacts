...
function NodeActionService(config) {
    ...
    const gcEveryTime = config.gcEveryTime;
    ...
    this.runCode = function runCode(req) {
        if (status === Status.ready && userCodeRunner !== undefined) {
            ...
            console.log("[profile] run handler begin");
            if (!ignoreRunStatus) {
            ...
            return doRun(msg).then(result => {
                ...
                if (typeof result !== 'object') {
                    ...
                    if (gcEveryTime)
                        global.gc();
                    console.log("[profile] zzmheapstat:" + JSON.stringify(v8.getHeapSpaceStatistics()));
                    console.log("[profile] run handler end");
                    const t1 = process.hrtime(t0)
                    console.log("[profile] execution time: " + ((t1[0] * 1000) + (t1[1] / 1000000)));
    ...
    this.reclaim =  function reclaim(req) {
        return new Promise((resolve, reject) => {
            const startTime = process.hrtime();
            v8.ZZMReclaimMemory();
            const endTime = process.hrtime(startTime);
            const elapsedTime = endTime[0] * 1e9 + endTime[1]; // 将秒数转换为纳秒数
            // global.gc();
            resolve(responseMessage(200, {"time": elapsedTime}));
        })
    }
