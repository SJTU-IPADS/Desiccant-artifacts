...
// <zzm>
void Generation::reclaimMemory(ContiguousSpace *space) {
  char *aligned_top = (char *)align_ptr_up((void *)space->top(), os::vm_page_size());
  char *aligned_end = (char *)align_ptr_down((void *)space->end(), os::vm_page_size());
  tty->print_cr("[ZZM] [ZZMReclaimMemory] [%p, %p, %p], mmap region: (%p,%p)",
         space->bottom(), space->top(), space->end(), aligned_top, aligned_end);

  void *ret = mmap((void *)aligned_top, aligned_end - aligned_top, PROT_READ|PROT_WRITE, MAP_PRIVATE|MAP_FIXED|MAP_NORESERVE|MAP_ANONYMOUS, -1, 0);
  if (ret != aligned_top) {
    printf("[ZZM] [ZZMReclaimMemory] [%p, %p, %p], bad mmap result (%p,%p), errno: %d (%s)\n",
           space->bottom(), space->top(), space->end(), aligned_top, aligned_end, errno, strerror(errno));
  }
}
// </zzm>
