...
void Universe::clear_clean_mapping() {
...
#define ZZM_RECLAIM_PORT 10086
    ...
    while (1) {
        ...
        long t0 = os::javaTimeNanos();
        Universe::clear_clean_mapping();   // merge later if needed, not for now due to the trick thread and safepoint problem. left this thread totally background
        long t1 = os::javaTimeNanos();
        char tmpbuffer[64];
        snprintf(tmpbuffer, sizeof(tmpbuffer), "%ld", t1 - t0);
        send_response(connfd, tmpbuffer, "text/plain");
        send(connfd, response, strlen(response), 0); close(connfd);
    }
}


void Universe::start_reclaim_server() {
    pthread_t tid;
