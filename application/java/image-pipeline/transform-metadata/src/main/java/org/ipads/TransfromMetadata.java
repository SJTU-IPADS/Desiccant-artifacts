...
//import java.funclet.FuncletContext;

public class TransfromMetadata {
    public static JsonObject main(JsonObject args) {
        JsonObject response = new JsonObject();

        JsonObject ret = new JsonObject();
        ret.add(ImageRecognizationCommons.IMAGE_NAME, args.get(ImageRecognizationCommons.IMAGE_NAME));

        args = args.getAsJsonObject(ImageRecognizationCommons.EXTRACTED_METADATA);
        //System.out.println("TransfromMetadata invoked");
        ...
        ret.add(ImageRecognizationCommons.EXTRACTED_METADATA, response);
        return ret;
    }
    ...
    public static void main(String args[])  {
        ...
        if (args.length > 0)
            executeCnt = Integer.parseInt(args[0]);


		int i = 0;
        FuncletContext context = Thread.currentThread().getFuncletContext(); 
        if (context == null) {
            while (executeCnt != 0) {
                i += 1;
                //System.out.println("=============execution" + String.valueOf(i) + " begin================");
                JsonObject request = new JsonParser().parse(s).getAsJsonObject();
                JsonObject result = main(request);
                //System.out.println("=============execution" + String.valueOf(i) + " done================");
