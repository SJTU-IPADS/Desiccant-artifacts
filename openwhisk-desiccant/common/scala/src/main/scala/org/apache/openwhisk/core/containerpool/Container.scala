...
case class ReclaimResult(reclaimedMemoryInKB: Int, reclaimTimeInMs: Double) {
}
...
trait Container {
  ...
  var pmapMemoryConsumption: ByteSize = new ByteSize(40, MB)
  var averageReclaimLeft = new ByteSize(30, MB)
  var reclaimCount: Int = 0

  var noNeedReclaim: Boolean = false
  var reclaimPending: Boolean = false

  def reclaim(timeout: FiniteDuration,
    maxConcurrent: Int = 1)(implicit transid: TransactionId): Future[Unit] = {
    val start =
      transid.started(
        this,
        LoggingMarkers.INVOKER_ACTIVATION_RUN,
        s"[zzm] reclaim memory at $id $addr",
        logLevel = InfoLevel)

    val t0 = System.nanoTime()
    logging.info(this, s"[debug] calling reclaim on container ${containerId}")
    callContainer("/reclaim", JsObject(), timeout, maxConcurrent, retry = true)
      .andThen { // never fails
        case Success(r: RunResult) =>
          r.response match {
            case Left(value) =>
              logging.info(this, s"extracted error: ${value.toString} on container ${containerId}")
              transid.failed(this, start, s"[zzm] reclaim failed with ${value.toString}, container ${containerId}")
            case Right(result) =>
              val reclaimTime1 = result.entity.parseJson.asJsObject().fields("time").convertTo[Long]
              logging.info(this, s"[zzm] reclaim1 get time: ${reclaimTime1} on container ${containerId}")
              callContainerRawReclaim(timeout)
                .andThen({
                  case Success(r: RunResult) =>
                    r.response match {
                      case Left(value) =>
                        logging.info(this, s"extracted error: ${value.toString} on container ${containerId}")
                        transid.failed(this, start, s"[zzm] reclaim2 failed with ${value.toString}, container ${containerId}")

                      case Right(result) =>
                        val reclaimTime2 = result.entity.toLong
                        logging.info(this, s"[zzm] reclaim2 get time: ${reclaimTime2} on container ${containerId}")
                        logging.info(this, s"[eval-cpu] gc-time:\t${reclaimTime1 + reclaimTime2} on container ${containerId}")
                        logging.info(this, s"[debug] reclaim result of container ${containerId} on container ${containerId}")
                        transid.finished(
                          this,
                          start.copy(start = r.interval.start),
                          s"[zzm] reclaim result: ${r.toBriefString}",
                          endTime = r.interval.end,
                          logLevel = InfoLevel)
                    }

                  case Failure(t) =>
                    logging.info(this, s"[debug] rawReclaim failed of container ${containerId}")
                    transid.failed(this, start, s"[zzm] rawReclaim failed with $t on container ${containerId}")

                })
              Future.unit
          }
        case Failure(t) =>
          transid.failed(this, start, s"[zzm] reclaim failed with $t on container ${containerId}")
      }
      .map { result =>
        if (result.interval.duration >= timeout) {
          // ActivationResponse.developerError(Messages.timedoutActivation(timeout, false))
          transid.failed(this, start, s"[zzm] reclaim failed with timeout: $timeout of container ${containerId}")
        }
      }
  }
  ...
  protected val rawReclaimPort = 10086;
  protected var rawReclaimConnection: Option[ContainerClient] = None

  protected def callContainerRawReclaim(
    timeout: FiniteDuration,
    retry: Boolean = false,
    reschedule: Boolean = false)(implicit transid: TransactionId): Future[RunResult] = {
    val started = Instant.now()
    val http = rawReclaimConnection.getOrElse {
      val conn = openConnections(timeout, 1, rawReclaimPort)
      rawReclaimConnection = Some(conn)
      conn
    }
    http
      .post("", JsObject(), retry, reschedule)
      .map { response =>
        val finished = Instant.now()
        RunResult(Interval(started, finished), response)
      }
  ...
  private def openConnections(timeout: FiniteDuration, maxConcurrent: Int, port:Int = addr.port) = {
    if (Container.config.akkaClient) {
      // logging.info(this, "[zzm] using akkaClient")
      ...
      // logging.info(this, s"[zzm] using apacheBlockingClient, maxConcurent: ${maxConcurrent}")
