...
case object Reclaiming extends ContainerState
...
case class WarmedData(override val container: Container,
                      ...
                      var lastFinished: Instant = Instant.now())
    extends ContainerStarted(container, lastUsed, action.limits.memory.megabytes.MB, activeActivationCount)
    with ContainerInUse {
...
case object Remove
case class Reclaim(expectedReclaimMemory: ByteSize = new ByteSize(0, SizeUnits.MB))
...
case class ReclaimComplete(beginNanoTime: Long, expectedReclaimSize: ByteSize)
case object ReclaimOngoing
case class MemoryUpdated(isReclaim: Boolean, expectedReclaimSize: ByteSize, data: WarmedData)
case class ReclaimRejected(expectedReclaimSize: ByteSize, hasBegin: Boolean = false, data: WarmedData)
...
class ContainerProxy(factory: (TransactionId,
  ...
  when(Uninitialized) {
    ...
    case Event(job: Start, _) =>
      factory(
        ...
        poolConfig.cpuCount(job.memoryLimit),
        None)
        .map(container =>
    ...
    case Event(job: Run, _) =>
      ...
      val container = factory(
        ...
        poolConfig.cpuCount(job.action.limits.memory.megabytes.MB),
        Some(job.action))
  ...
  when(Started) {
    ...
    case Event(Remove, data: PreWarmedData) => {
      logging.info(this, "destroyContainer 1")
    ...
    case Event(_: FailureMessage, data: PreWarmedData) =>
      ...
      logging.info(this, "destroyContainer 2")
...
//      data.container.noNeedReclaim = false
    ...
    case Event(FailureMessage(e: ContainerHealthError), data: WarmedData) =>
      ...
      rescheduleJob = true
      rejectBuffered()
      logging.info(this, "destroyContainer 3")
    ...
    case Event(f: FailureMessage, data: PreWarmedData) =>
      ...
      logging.info(this, "destroyContainer 4")
    ...
    case Event(_: FailureMessage, data: WarmedData) =>
      ...
      activeCount -= 1
      if (activeCount == 0) {
        logging.info(this, "destroyContainer 5")
        destroyContainer(data, true)
      } else {
    ...
    case Event(Reclaim(expectedReclaimMemory), data: WarmedData) => {
      context.parent ! ReclaimRejected(expectedReclaimMemory, false, data)
      stay()
    }
    case Event(ReclaimComplete(t0, expectedReclaimMemory), data: WarmedData) => {
      context.parent ! ReclaimRejected(expectedReclaimMemory, true, data)
      stay()
    }
  ...
  when(Ready, stateTimeout = pauseGrace) {
    ...
    case Event(StateTimeout, data: WarmedData) =>
      val f1 = data.container.suspend()(TransactionId.invokerNanny)
      val dockerContainer = data.container.asInstanceOf[DockerContainer]
      val f2 = dockerContainer.updatePmapMemoryConsumption()(TransactionId.invokerNanny)
        .map(_ => {
//          logging.info(this, s"[zzm] updating memory for paused container ${data.container.containerId}, data: ${data}(${data.hashCode()}),origMem: ${data.realMemoryConsumption}, newMem ${dockerContainer.pmapMemoryConsumption}")
//          data.realMemoryConsumption = dockerContainer.pmapMemoryConsumption
//          logging.info(this, s"[zzm] updated memory for paused container ${data.container.containerId}, data: ${data}(${data.hashCode()}),origMem: ${data.realMemoryConsumption}, newMem ${dockerContainer.pmapMemoryConsumption}")
//          logging.info(this, s"[zzm] updated memory for paused container ${data.container.containerId}, newMem ${dockerContainer.pmapMemoryConsumption}")
//          logging.info(this, s"[zzm] sending MemoryUpdated to container pool after pause ${context.parent}")
          context.parent ! MemoryUpdated(false, null, null)
        })
      Future.sequence(List(f1, f2)).map(_ =>  {
        logging.info(this, s"[zzm] container ${data.container.containerId} sending paused to self")
        ContainerPaused
      }).pipeTo(self)
      goto(Pausing)

    case Event(Remove, data: WarmedData) =>
      logging.info(this, "destroyContainer 6")
      destroyContainer(data, true)

    // warm container failed
    case Event(_: FailureMessage, data: WarmedData) =>
      logging.info(this, "destroyContainer 7")
      destroyContainer(data, true)
  ...
  when(Pausing) {
    ...
    case Event(msg: FailureMessage, data: WarmedData) =>
      msg.cause.printStackTrace()
      logging.info(this, s"destroyContainer 8, msg ${msg.cause}")
      destroyContainer(data, true)
  ...
  when(Paused, stateTimeout = unusedTimeout) {
    case Event(job: Run, data: WarmedData) =>
      ...
      data.container
        ...
        .flatMap(_ => {
//          context.parent ! MemoryUpdated(false)
          initializeAndRun(data.container, job, true)
        }).map(_ => RunCompleted)
        .pipeTo(self)
      goto(Running) using newData
...
//      logging.info(this, "destroyContainer 9")

      destroyContainer(data, true)

    case Event(Reclaim(expectedReclaimMemory), data: WarmedData) =>
      context.parent ! ReclaimOngoing
      implicit val transid = TransactionId.invoker
      val dockerContainer =  data.container.asInstanceOf[DockerContainer]
      dockerContainer.resume().onComplete { _ =>
        val t0 = System.nanoTime()
        dockerContainer.reclaim(defaultTimeOut)
          .map(_ => ReclaimComplete(t0, expectedReclaimMemory)).pipeTo(self)
      }
    goto (Reclaiming) using data
  ...
  when (Reclaiming) {
    ...
    case Event(ReclaimComplete(t0, expectedReclaimSize), data:WarmedData) =>
      val dockerContainer = data.container.asInstanceOf[DockerContainer]
//      dockerContainer.noNeedReclaim = true
      val t1 = System.nanoTime()
//      val oldMemConsumption = dockerContainer.pmapMemoryConsumption
      dockerContainer.updatePmapMemoryConsumption()(TransactionId.invokerNanny)
        .map(_ => {
          try {
//            var curReclaimEffiency = 0.0
//            if (oldMemConsumption < dockerContainer.pmapMemoryConsumption) {
//              logging.warn(this, s"[zzm] after reclaim complete, oldMem: ${oldMemConsumption}, newMem: ${dockerContainer.pmapMemoryConsumption}, t1:${t1}, t0:${t0}, memdiff is Negative, timediff ${((t1 - t0) / 1000)}")
//            } else {
//              logging.info(this, s"[zzm] after reclaim complete, oldMem: ${oldMemConsumption}, newMem: ${dockerContainer.pmapMemoryConsumption}, t1:${t1}, t0:${t0}, memdiff: ${(oldMemConsumption - dockerContainer.pmapMemoryConsumption).toKB}, timediff ${((t1 - t0) / 1000)}")
//              curReclaimEffiency = (oldMemConsumption - dockerContainer.pmapMemoryConsumption).toKB.toDouble / ((t1 - t0) / 1000)
//            }
            var curReclaimLeft = dockerContainer.pmapMemoryConsumption
            context.parent ! MemoryUpdated(true, expectedReclaimSize, data)
            dockerContainer.averageReclaimLeft =
              ((dockerContainer.averageReclaimLeft * dockerContainer.reclaimCount) + curReclaimLeft) / (dockerContainer.reclaimCount + 1)
//            dockerContainer.averageReclaimEffiency =
//              ((dockerContainer.averageReclaimEffiency * dockerContainer.reclaimCount) + curReclaimEffiency) / (dockerContainer.reclaimCount + 1)
            dockerContainer.reclaimCount += 1
          } catch  {
            case ex: Throwable => ex.printStackTrace()
          }
        })
      data.container.suspend()(TransactionId.invokerNanny)
        .map(_ => ContainerPaused).pipeTo(self)
      goto(Pausing) using data

    case Event(StateTimeout | Remove, data: WarmedData) =>
      logging.info(this, "destroyContainer 10")
      destroyContainer(data, true)
  }
  ...
  when(Removing) {
    ...
    case Event(RunCompleted, data: WarmedData) =>
      ...
      //if there are items in runbuffer, process them if there is capacity, and stay; otherwise if we have any pending activations, also stay
      if (activeCount == 0) {
        logging.info(this, "destroyContainer 11")
        destroyContainer(newData, true)
      } else {
        stay using newData
      }
    ...
    case Event(_: FailureMessage, data: WarmedData) =>
      ...
      val newData = data.withoutResumeRun()
      if (activeCount == 0) {
        logging.info(this, "destroyContainer 12")
        destroyContainer(newData, true)
      } else {
        stay using newData
      }
...
//    new Exception("destroy Container called!").printStackTrace()
