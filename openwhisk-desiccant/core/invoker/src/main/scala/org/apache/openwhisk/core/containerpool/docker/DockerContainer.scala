...
object DockerContainer {
  ...
  def create(transid: TransactionId,
             ...
             cpuCount: Double = 0.0)(implicit docker: DockerApiWithFileAccess,
                                                            runc: RuncApi,
                                                            as: ActorSystem,
                                                            ec: ExecutionContext,
    ...
    val cpuConfig = if (cpuCount == 0.0) Seq("--cpu-shares", cpuShares.toString) else Seq("--cpus", "%.4f".format(cpuCount))
    val args = cpuConfig ++
      Seq(
      "--memory",
      s"${memory.toMB}m",
    ...
    for {
      ...
      (ip, pid) <- docker.inspectIPAddress(id, network).recoverWith {
        // remove the container immediately if inspect failed as
        // we cannot recover that case automatically
        case _ =>
          docker.rm(id)
          Future.failed(WhiskContainerStartupError(Messages.resourceProvisionError))
      }.zip(docker.getMainRuntimePID(id, imageToUse))
    } yield new DockerContainer(id, ip, useRunc, pid)
  }
}
...
class DockerContainer(protected val id: ContainerId,
                      ...
                      val mainRuntimePID : String = "")(implicit docker: DockerApiWithFileAccess,
                                                      runc: RuncApi,
  ...
  protected var pmapHttpConnection: Option[ContainerClient] = None
  val pmapServerAddress = "192.168.22.156:13579"

  def updatePmapMemoryConsumption()(implicit transid: TransactionId): Future[Unit] = {
    logging.info(this, s"[zzm] updatePmapMemoryConsumption of container ${id}, mainRuntime: ${mainRuntimePID}")
    val timeout = docker.asInstanceOf[DockerClient].defaultTimeOut

    val start = transid.started(
      this,
      LoggingMarkers.INVOKER_DOCKER_CMD("pmap"),
      s"[zzm] getting pmap of ${mainRuntimePID}",
      logLevel = InfoLevel)

    val http = pmapHttpConnection.getOrElse {
      val conn = new ApacheBlockingContainerClient(
        ...
        100)
      pmapHttpConnection = Some(conn)
      conn
    }
    logging.info(this, s"get pmapResult response from ${http.toString}, pid: ${mainRuntimePID.toString}, container: ${containerId}")
    http
      .post("PID=" + mainRuntimePID.toString, JsNull, false, false)
      .flatMap { response =>
        logging.info(this, s"response of updatePostResponse: ${response.toString}")
        response match {
          case Left(value) =>
            logging.info(this, s"extracted error: ${value.toString}")
            Future.unit
          case Right(result) =>
            logging.info(this, s"[zzm] read from pmap helper, got result: ${result}")
            pmapMemoryConsumption =  new ByteSize(result.entity.toLong, KB)
            logging.info(this, s"[zzm] read from pmap helper, updated pmem size: ${pmapMemoryConsumption.toMB} M")
            Future.unit
        }
      }
  }
  ...
  override def suspend()(implicit transid: TransactionId): Future[Unit] = {
    super.suspend().flatMap(_ => {
      val pauseFuture = if (useRunc) runc.pause(id) else docker.pause(id)
      pauseFuture
    })
  }
