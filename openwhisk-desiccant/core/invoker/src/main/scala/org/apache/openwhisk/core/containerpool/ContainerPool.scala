...
class ContainerPool(childFactory: ActorRefFactory => ActorRef,
    ...
    extends Actor {
  import ContainerPool.memoryConsumptionOfBusyPool
  import ContainerPool.memoryConsumptionOfFreePool
  ...
  context.system.scheduler.schedule(2.seconds, 2.seconds, self, IncreaseThreshold)
  ...
  def reclaimAllInPool(): Unit = {
    logging.info(this, "reclaimAllInPool called")
    for (elem <- freePool) {
      elem match {
        case (ref, w: WarmedData) if w.activeActivationCount == 0 =>
          ref ! Reclaim(new ByteSize(0, SizeUnits.MB));
      }
    }
  }

  def removeAllInPool(): Unit = {
    for (elem <- busyPool) {
      elem match {
        case (ref, w: WarmedData) if w.activeActivationCount == 0 =>
          ref ! Remove;
    ...
    freePool = immutable.Map.empty[ActorRef, ContainerData]
    busyPool = immutable.Map.empty[ActorRef, ContainerData]
  }
  ...
  def receive: Receive = {
    ...
    case r: Run =>
//      if (r.action.name.asString.equals("ZZMReclaimAll")) {
//        reclaimAllInPool();
//      } else {
        ...
        if (runBuffer.isEmpty || isResentFromBuffer) {
          ...
          val createdContainer = {
            ...
            logging.info(this, s"[zzm] new invocation request ${r.action.name.asString}, busyPool: ${busyPool.size} (${memoryConsumptionOfBusyPool(busyPool)} MB), freePool: ${freePool.size} (${memoryConsumptionOfFreePool(freePool)}MB)")
            if (hasPoolSpaceFor(busyPool, r.action.limits.memory.megabytes.MB)) {
              logging.info(this, s"[zzm] maxinum: ${poolConfig.userMemory.toMB}, new action requires: ${r.action.limits.memory.megabytes.MB.toMB}")
              // Schedule a job to a warm container
              ContainerPool
                .schedule(r.action, r.msg.user.namespace.name, freePool)
                .map(container => {
                  logging.info(this, s"[zzm] use existing free container ${container._2.getContainer.get.containerId} for ${r.action.name.asString}")
                  logging.info(this, s"[evaluation] warm start for app:${r.action.name.asString}")
                  (container, container._2.initingState)
                ...
                .orElse(
                  ...
                  if (hasPoolSpaceFor(busyPool, r.action.limits.memory.megabytes.MB)) {
                    takePrewarmContainer(r.action)
                      ...
                      .orElse {
                        logging.info(this, s"[zzm] create new contaienr for ${r.action.name.asString}")
                        logging.info(this, s"[evaluation] cold start for app:${r.action.name.asString}")
              ...
              //                  .remove(freePool, Math.min(r.action.limits.memory.megabytes, memoryConsumptionOfFreePool(freePool)).MB)
              //                  .map(removeContainer)
          ...
          createdContainer match {
            case Some(((actor, data), containerState)) =>
              ...
              if (!newData.hasCapacity()) {
                ...
                logging.error(this, "[fatalerror] hasCapacity after nextRun should never happen!!")
              ...
              val retryLogDeadline = if (isErrorLogged) {
                logging.warn(
                  ...
                  s"Rescheduling Run message, too many message in the pool, " +
                    s"freePoolSize: ${freePool.size} containers and ${memoryConsumptionOfFreePool(freePool)} MB, " +
                    s"busyPoolSize: ${busyPool.size} containers and ${memoryConsumptionOfBusyPool(busyPool)} MB, " +
...
//      }
    ...
    case NeedWork(warmData: WarmedData) =>
      ...
      newData.container.noNeedReclaim = false
      newData.lastFinished = Instant.now()
      ...
      if (newData.hasCapacity()) {
        ...
        insertIntoFreePool(sender(), newData)
//        freePool = freePool + (sender() -> newData)
      ...
      if (warmData.action.name.asString.equals("ZZMReclaimAll")) {
        reclaimAllInPool();
      } else if (warmData.action.name.asString.equals("ZZMRemoveAll")) {
        removeAllInPool();
      }
      processBufferOrFeed()
    ...
    case IncreaseThreshold =>
      if (poolConfig.reclaimThreshold < 0.9)
        poolConfig.reclaimThreshold += 0.05

    case ReclaimRejected(expectedReclaimSize, hasBegin, data) =>
      if (!hasBegin)
        pendingReclaimingCnt -= 1
      else
        reclaimingCnt -= 1
      expectedPendingReclaimSize -= expectedReclaimSize
      data.container.reclaimPending = false
      data.container.noNeedReclaim = false

    case MemoryUpdated(isReclaim, expectedReclaimSize, data) =>
//      logging.info(this, s"[zzm] container Pool received memory updated request, is Reclaim: ${isReclaim}, freePool: ${freePool.size} (${memoryConsumptionOfFreePool(freePool)}MB)")
//      logging.info(this, "[zzm] now dump freePool")
//      var curFree = 0
//      freePool.foreach { item =>
//        logging.info(this, s"[${curFree}] actor: ${item._1}, data: ${item._2} (${item._2.hashCode()}, container: ${item._2.getContainer.get.containerId}, containerMem: ${item._2.getContainer.get.pmapMemoryConsumption.toMB} MB" )
//        curFree += 1
//      }

      if (isReclaim) {
        logging.info(this, s"decrease reclaiming count ${reclaimingCnt} -> ${reclaimingCnt - 1}")
        data.container.noNeedReclaim = true
        data.container.reclaimPending = false
        reclaimingCnt -= 1
        expectedPendingReclaimSize -= expectedReclaimSize
      }
      if (freePool.nonEmpty)
        reclaimIfNeeded(memoryConsumptionOfFreePool(freePool))

    case ReclaimOngoing =>
      logging.info(this, s"increase reclaiming count ${reclaimingCnt} -> ${reclaimingCnt + 1}")
      pendingReclaimingCnt -= 1
      reclaimingCnt += 1
  }
...
//    freePool = freePool + (ref -> data)
  ...
  def removeContainer(toDelete: ActorRef) = {
    logging.info(this, s"[evict] totally evicting container: ${toDelete}")
    toDelete ! Remove
   ...
   */
  def hasPoolSpaceFor[A](localBusyPool: Map[A, ContainerData], memory: ByteSize): Boolean = {
    val totalConsumption = memoryConsumptionOfBusyPool(localBusyPool) + memory.toMB
    logging.info(this, s"[zzm] hasPoolSpaceFor ${memory}, totalConsumption: ${totalConsumption}")
    totalConsumption <= poolConfig.busyPoolLimit.toMB
  }


  def hasPoolSpaceFor[A](localFreePool: Map[A, ContainerData], localBusyPool: Map[A, ContainerData], memory: ByteSize): Boolean = {
    val freePoolConsumption = memoryConsumptionOfFreePool(freePool)
    val busyPoolConsumption = memoryConsumptionOfBusyPool(busyPool)
    val totalConsumption = freePoolConsumption + busyPoolConsumption + memory.toMB
    logging.info(this, s"[zzm] hasPoolSpaceFor ${memory}, totalConsumption: ${totalConsumption}, freeConsumption ${freePoolConsumption}, busyConsumption ${busyPoolConsumption}")
//    if (freePool.nonEmpty)
//      reclaimIfNeeded(freePoolConsumption, busyPoolConsumption + memory.toMB)
    totalConsumption <= poolConfig.userMemory.toMB
  }

  def insertIntoFreePool(act: ActorRef, data: ContainerData): Unit = {
    val freePoolConsumption = memoryConsumptionOfFreePool(freePool)
    val addedPmap = data.getContainer.get.pmapMemoryConsumption.toMB
    logging.info(this, s"[zzm] [freepool] hasPoolSpaceFor ${addedPmap}, totalConsumption: ${freePoolConsumption}, limit: ${poolConfig.freePoolMemory.toMB}")
    if (freePool.nonEmpty)
      reclaimIfNeeded(freePoolConsumption)
    if (freePoolConsumption + addedPmap > poolConfig.freePoolLimit.toMB) {
      poolConfig.reclaimThreshold = poolConfig.initialReclaimThreshold
      ContainerPool
        .remove(freePool, Math.min(addedPmap, freePoolConsumption).MB)
        .map(removeContainer)
    }
    freePool = freePool + (act -> data)
  }
  ...
  val defaultTimeOut: FiniteDuration = new FiniteDuration(1, TimeUnit.SECONDS);

  var pendingReclaimingCnt: Int = 0
  var reclaimingCnt: Int = 0
  var expectedPendingReclaimSize: ByteSize = 0.MB
  var expectedReclaimErrorRate: Double = 0.8

  def reclaimIfNeeded(freeMemoryInMB: Long): Unit = {
    val toReclaim = freeMemoryInMB - (poolConfig.freePoolLimit.toMB * poolConfig.reclaimThreshold).toLong
    logging.info(this, s"[zzm] reclaimIfNeeded called, freeMemory: ${freeMemoryInMB},  threshold: ${(poolConfig.freePoolLimit.toMB * poolConfig.reclaimThreshold).toLong}, rate: ${poolConfig.reclaimThreshold}, toRecaim: ${toReclaim}")
    if (toReclaim > expectedPendingReclaimSize.toMB * expectedReclaimErrorRate) {
      val idleCPU = poolConfig.cpuCount(poolConfig.userMemory - memoryConsumptionOfBusyPool(busyPool).MB)
      reclaimUpTo(toReclaim.MB, idleCPU)
    }
  }

  def reclaimUpTo(memory: ByteSize, idleCPU: Double): Unit = {
    val maxPickCnt = idleCPU - reclaimingCnt - pendingReclaimingCnt
    logging.info(this, s"[zzm] reclaim with target memory: ${memory}, idleCPU: ${idleCPU}, pendingReclaimingCnt: ${pendingReclaimingCnt},reclaimingCnt: ${reclaimingCnt}, expectedPendingReclaimSize: ${expectedPendingReclaimSize}")
    if (maxPickCnt <= 0)
      return
    val sortedItems = new mutable.TreeMap[Double, ListBuffer[(ActorRef, WarmedData)]]
    for (elem <- freePool) {
      elem match {
        case (ref, w: WarmedData) if w.activeActivationCount == 0 =>
//          logging.info(this, s"trying to pick container ${w.container.containerId}, noNeedReclaim: ${w.container.noNeedReclaim}, lastUsed: ${w.lastUsed}, peroid: ${java.time.Duration.between(w.lastUsed, Instant.now()).toMillis}, theshold: ${poolConfig.minReclaimDurationInMs}")
          if (!w.container.noNeedReclaim && !w.container.reclaimPending && java.time.Duration.between(w.lastFinished, Instant.now()).toMillis > poolConfig.minReclaimDurationInMs) {
            val dockerContainer = w.container.asInstanceOf[DockerContainer]
            var expectedReclaimSizeInKB = dockerContainer.pmapMemoryConsumption.toKB - dockerContainer.averageReclaimLeft.toKB
            if (expectedReclaimSizeInKB <= 0)
              expectedReclaimSizeInKB = 5*1024
            val curItem = sortedItems.getOrElseUpdate(expectedReclaimSizeInKB, new ListBuffer[(ActorRef, WarmedData)])
            curItem.append((ref, w))
          }
      }
    }

    var continueReclaim = true;
    val iter = sortedItems.toList.reverseIterator
    while (continueReclaim && iter.hasNext) {
      val item = iter.next()
      val expectedReclaimSize = new ByteSize(item._1.toLong, SizeUnits.KB)
      val iter2 = item._2.iterator
      while (continueReclaim && iter2.hasNext) {
        val actorTuple = iter2.next()
        actorTuple._1 ! Reclaim(expectedReclaimSize)
        val w = actorTuple._2
        w.container.reclaimPending = true
        logging.info(this, s"[debug] pool state pre : pendingReclaimCnt: ${pendingReclaimingCnt}, expectedPendingReclaimSize: ${expectedPendingReclaimSize}, reclaimingCnt: ${reclaimingCnt}")
        logging.info(this, s"[debug] picked container ${w.container.containerId}, noNeedReclaim: ${w.container.noNeedReclaim}, expectedReclaimableInKB: ${item._1}")
        expectedPendingReclaimSize += expectedReclaimSize
        pendingReclaimingCnt += 1
        logging.info(this, s"[debug] pool state post : pendingReclaimCnt: ${pendingReclaimingCnt}, expectedPendingReclaimSize: ${expectedPendingReclaimSize}, reclaimingCnt: ${reclaimingCnt}")
        if (idleCPU < (reclaimingCnt + pendingReclaimingCnt) || memory.toMB < expectedPendingReclaimSize.toMB * expectedReclaimErrorRate) {
          logging.info(this, s"[debug] pool state stop : pendingReclaimCnt: ${pendingReclaimingCnt}, expectedPendingReclaimSize: ${expectedPendingReclaimSize}, reclaimingCnt: ${reclaimingCnt}")
          continueReclaim = false;
        }
      }
    }
    logging.info(this, s"[debug] pool state stopped : pendingReclaimCnt: ${pendingReclaimingCnt}, expectedPendingReclaimSize: ${expectedPendingReclaimSize}, reclaimingCnt: ${reclaimingCnt}")
  }
}
...
object ContainerPool {
   ...
   */
  protected[containerpool] def memoryConsumptionOfBusyPool[A](pool: Map[A, ContainerData]): Long = {
    pool.map(_._2.memoryLimit.toMB).sum
  }
  protected[containerpool] def memoryConsumptionOfFreePool[A](pool: Map[A, ContainerData]): Long = {
    pool.map(_._2.getContainer.get.pmapMemoryConsumption.toMB).sum
  }
  ...
  protected[containerpool] def remove[A](pool: Map[A, ContainerData],
    ...
    if (memory > 0.B && freeContainers.nonEmpty && memoryConsumptionOfFreePool(freeContainers) >= memory.toMB) {
      ...
      val remainingMemory = Try(memory - data.container.pmapMemoryConsumption).getOrElse(0.B)
