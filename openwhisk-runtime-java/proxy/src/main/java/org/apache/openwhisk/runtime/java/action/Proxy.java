...
public class Proxy {
    ...
    private boolean eagerGC = false;


    public Proxy(int port, boolean eagerGC) throws IOException {
        this.server = HttpServer.create(new InetSocketAddress(port), -1);
        this.eagerGC = eagerGC;

        this.server.createContext("/init", new InitHandler());
        this.server.createContext("/run", new RunHandler());
        this.server.createContext("/reclaim", new ReclaimHandler());
    ...
    private class RunHandler implements HttpHandler {
        public void handle(HttpExchange t) throws IOException {
            System.out.println("[profile] run handler begin");
            ...
            try {
                ...
                System.out.println("[profile] invokeMain time: " + String.valueOf(t01 - t00));
                ...
                System.out.println("[profile] run handler end, total time: " + String.valueOf(t1 - t0));
            }
        }
    }

    private class ReclaimHandler implements HttpHandler {
        public void handle(HttpExchange t) throws IOException {
            System.out.println("[profile] reclaim handler begin");
            long t0 = System.nanoTime();
            System.ZZMReclaimMemory();
            long t1 = System.nanoTime();
            Proxy.writeResponse(t, 200, "{\"time\": " + String.valueOf(t1 - t0)+ " }" );
        }
    }

    public static void main(String args[]) throws Exception {
        Proxy proxy = new Proxy(8080, args[0].equals("gc"));
        proxy.start();
    }
}
