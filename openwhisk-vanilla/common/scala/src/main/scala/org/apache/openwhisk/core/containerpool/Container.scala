...
trait Container {
  ...
  var averageReclaimEffiency: Double = 0
      ...
      .andThen { // never fails
        case Success(r: RunResult) =>
          transid.finished(
            ...
            logLevel = InfoLevel)
        case Failure(t) =>
          transid.failed(this, start, s"[zzm] reclaim failed with $t")
      }
      .map { result =>
        if (result.interval.duration >= timeout) {
          // ActivationResponse.developerError(Messages.timedoutActivation(timeout, false))
          transid.failed(this, start, s"[zzm] reclaim failed with timeout: $timeout")
        }
      }
  }
