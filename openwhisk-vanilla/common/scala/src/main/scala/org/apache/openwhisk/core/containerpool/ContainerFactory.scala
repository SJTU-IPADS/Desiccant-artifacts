...
case class ContainerPoolConfig(userMemory: ByteSize,
  ...
  val initialReclaimThreshold : Double = 2.0
  val minReclaimDurationInMs : Double = 100
  var reclaimThreshold: Double = initialReclaimThreshold

  val busyPoolLimit: ByteSize = userMemory
  val freePoolLimit: ByteSize = freePoolMemory

  private val memoryPerCPU = 1769.0
  private val totalCPUCount = (userMemory.toMB / memoryPerCPU).toInt
  ...
  def cpuCount(reservedMemory: ByteSize) =
    reservedMemory.toMB / memoryPerCPU
}
...
trait ContainerFactory {
  ...
  def createContainer(
    ...
    cpuCount: Double,
    action: Option[ExecutableWhiskAction])(implicit config: WhiskConfig, logging: Logging): Future[Container] = {
    createContainer(tid, name, actionImage, userProvidedImage, memory, cpuShares, cpuCount)
  }
  ...
  def createContainer(tid: TransactionId,
    ...
    memory: ByteSize,
    cpuShares: Int,
    cpuCount: Double)(implicit config: WhiskConfig, logging: Logging): Future[Container] = {
    createContainer(tid, name, actionImage, userProvidedImage, memory, cpuShares)
  }
