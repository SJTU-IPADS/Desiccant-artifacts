    ...
    Try(connection.execute(request)).map { response =>
      val containerResponse = Option(response.getEntity)
        .map { entity =>
          ...
          //logging.info(this, s"ApacheBlockingContainerClient execute, request to ${request.getURI}, response entity: ${entity.toString}, contentLength: ${entity.getContentLength}")
          ...
          if (contentLength >= 0) {
            ...
            //logging.info(this, s"gotten str without content length: ${str}")
            if (str.length > 0)
              Right(ContainerResponse(statusCode, str, None))
            else
