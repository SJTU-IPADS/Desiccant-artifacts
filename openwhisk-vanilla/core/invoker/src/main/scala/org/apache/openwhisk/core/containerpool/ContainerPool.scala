...
//        freePool = freePool + (sender() -> newData)
      ...
      if (warmData.action.name.asString.equals("ZZMRemoveAll")) {
        removeAllInPool()
      }
      processBufferOrFeed()
...
//        reclaimIfNeeded(memoryConsumptionOfFreePool(freePool), memoryConsumptionOfBusyPool(busyPool))
...
//      reclaimIfNeeded(freePoolConsumption, busyPoolConsumption + memory.toMB)
  ...
  def reclaimIfNeeded(freeMemoryInMB: Long, busyMemoryInMB: Long): Unit = {
    val toReclaim =  (freeMemoryInMB + busyMemoryInMB - (poolConfig.userMemory.toMB * poolConfig.reclaimThreshold)).toLong
    logging.info(this, s"[zzm] reclaimIfNeeded called, freeMemory: ${freeMemoryInMB}, busyMemory: ${busyMemoryInMB}, threshold: ${poolConfig.userMemory.toMB * poolConfig.reclaimThreshold},toRecaim: ${toReclaim}")
    if (toReclaim > 0) {
      val idleCPU = poolConfig.cpuCount(poolConfig.userMemory - busyMemoryInMB.MB)
      reclaimUpTo(toReclaim.MB, idleCPU)
    }
  }

  def reclaimUpTo(memory: ByteSize, idleCPU: Double): Unit = {
    // memory seems uesless, as it is hard to predict...
    // may be need the predict
    val pickCnt = idleCPU - reclaimingCnt
    logging.info(this, s"[zzm] reclaim with target memory: ${memory}, idleCPU: ${idleCPU}, reclaimingCnt: ${reclaimingCnt}, pickCnt: ${pickCnt}")
    if (pickCnt <= 0)
      return
    val mins = new mutable.TreeMap[Double, ListBuffer[(ActorRef, WarmedData)]]
    var insertSize = 0
    for (elem <- freePool) {
      elem match {
        case (ref, w: WarmedData) if w.activeActivationCount == 0 =>
//          logging.info(this, s"trying to pick container ${w.container.containerId}, noNeedReclaim: ${w.container.noNeedReclaim}, lastUsed: ${w.lastUsed}, peroid: ${java.time.Duration.between(w.lastUsed, Instant.now()).toMillis}, theshold: ${poolConfig.minReclaimDurationInMs}")
          if (!w.container.noNeedReclaim && java.time.Duration.between(w.lastUsed, Instant.now()).toMillis > poolConfig.minReclaimDurationInMs) {
//            logging.info(this, s"${w.container.containerId} considered of being reclaimed, insertCnt: ${insertSize}, pickCnt: ${pickCnt}")
            val averageReclaimEffiency = w.container.asInstanceOf[DockerContainer].averageReclaimEffiency
            val emptyListBuffer = new ListBuffer[(ActorRef, WarmedData)]
            if (insertSize < pickCnt) {
              val curItem = mins.getOrElseUpdate(averageReclaimEffiency, emptyListBuffer)
              curItem.append((ref, w))
              insertSize += 1
            } else {
              val curMax = mins.last
              if (curMax._1 > averageReclaimEffiency) {
                if (curMax._2.size == 1) {
                  mins.remove(curMax._1)
                } else {
                  curMax._2.remove(0)
                }
                val curItem = mins.getOrElseUpdate(averageReclaimEffiency, emptyListBuffer)
...
//          else {
//            logging.info(this, s"${w.container.containerId} not considered of being reclaimed")
//          }
      }
    }
    logging.info(this, s"[zzm] freePool: ${freePool.size} containers, picked ${insertSize} containers")
    mins.foreach { item =>
      item._2.foreach( item => {
        logging.info(this, s"[zzm] sending reclaim to container: ${item._2.container.containerId}")
        item._1 ! Reclaim
      })
    }
  }
