...
class DockerClient(dockerHost: Option[String] = None,
  ...
  override def getMainRuntimePID(id: ContainerId, image: String)(implicit transid: TransactionId): Future[String] = {
    runCmd(Seq("inspect", id.asString, "--format", "{{.State.Pid}}"), config.timeouts.inspect)
  }
...
trait DockerApi {
  ...
  def getMainRuntimePID(id: ContainerId, image: String)(implicit transid: TransactionId): Future[String] = {
    Future.successful("")
  }
}
