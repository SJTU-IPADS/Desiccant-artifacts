...
case class ReclaimComplete(beginNanoTime: Long)
case object ReclaimOngoing
case class MemoryUpdated(isReclaim: Boolean)
...
//      logging.info(this, "destroyContainer 9")
    ...
    case Event(Reclaim, data: WarmedData) =>
//      context.parent ! ReclaimOngoing
      ...
      dockerContainer.resume().onComplete { _ =>
        ...
        dockerContainer.reclaim(defaultTimeOut)
          .map(_ => ReclaimComplete(t0)).pipeTo(self)
      }
  ...
  when (Reclaiming) {
    case Event(job: Run, data: WarmedData) =>
      ...
      activeCount += 1
      val newData = data.withResumeRun(job)
//      context.parent ! MemoryUpdated(true)
      initializeAndRun(data.container, job, true)
        .map(_ => RunCompleted)
        .pipeTo(self)
      goto(Running) using newData

    case Event(ReclaimComplete(t0), data:WarmedData) =>
      val dockerContainer = data.container.asInstanceOf[DockerContainer]
