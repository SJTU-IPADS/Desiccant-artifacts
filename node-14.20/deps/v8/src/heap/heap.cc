...
bool Heap::zzm_is_mild_gc = false;  //<zzm>
...
void Heap::PrintShortHeapStatistics() {
  ...
  PrintIsolate(isolate_,
               ...
               read_only_space_->CommittedPhysicalMemory() / KB);
  PrintIsolate(isolate_,
  ...
  PrintIsolate(isolate_,
               ...
               new_lo_space_->CommittedPhysicalMemory() / KB);
  PrintIsolate(isolate_,
               ...
               old_space_->CommittedPhysicalMemory() / KB);
  PrintIsolate(isolate_,
  ...
  PrintIsolate(isolate_,
               ...
               code_lo_space_->CommittedPhysicalMemory() / KB);
  ...
  PrintIsolate(isolate_,
               ...
               (this->CommittedPhysicalMemory() + ro_space->CommittedPhysicalMemory()) / KB);
  PrintIsolate(isolate_,
...
// <zzm>
size_t Heap::ZZMGcMemoryReclaimed() {
  // should record reclaimed memory at the correct point
  // only encounter explict GC reclaimed memory
  return 0;
}

size_t Heap::ZZMGcCpuCost() {
  // should record CPU cost at the correct point
  // only encounter explict GC reclaimed memory
  // how to account cpu core count? single threaded?
  return 0;
}

void Heap::ZZMReclaimMemory() {
  PrintIsolate(isolate(), "[zzm] ZZMReclaimMemory entered\n");
  // CollectAllAvailableGarbage(GarbageCollectionReason::kTesting);
  zzm_is_mild_gc = true;
  PreciseCollectAllGarbage(Heap::kNoGCFlags,
                                 GarbageCollectionReason::kTesting,
                                 kGCCallbackFlagForced);
  // free from space
  UncommitFromSpace();
  // free to space
  int cnt = new_space_->to_space().pages_used();
  Page *current_page = new_space_->to_space().current_page();
  if (new_space_->limit() != current_page->area_end()) {
    printf("[zzm] ZZMReclaimMemory, bad thing happened, new_space allocation info: [%p, %p], current_page: %p [%p, %p]\n",
           new_space_->top(), new_space_->limit(), current_page, current_page->area_start(), current_page->area_end());
  }

  Address current_page_uncommit_begin = new_space_->top();
  while (true) {
    current_page_uncommit_begin = RoundUp(current_page_uncommit_begin, 4*1024);   // ugly...
    printf("[ZZM] [ZZMReclaimMemory] [%p, %p, %lu] begin\n", current_page, (void *)current_page_uncommit_begin, current_page->area_end() - current_page_uncommit_begin);
    if (!(current_page_uncommit_begin >= current_page->area_start() && current_page_uncommit_begin <= current_page->area_end())) {
      printf("[zzm] [ZZMReclaimMemory], bad thing happened, uncommit begin: %p, current_page: %p [%p, %p]\n",
             current_page_uncommit_begin, current_page, current_page->area_start(), current_page->area_end());
    }
    void * ret = mmap((void *)current_page_uncommit_begin, current_page->area_end() - current_page_uncommit_begin, PROT_READ|PROT_WRITE, MAP_PRIVATE|MAP_FIXED|MAP_NORESERVE|MAP_ANONYMOUS, -1, 0);
    if (reinterpret_cast<intptr_t>(ret)  != current_page_uncommit_begin) {
      printf("[ZZM] [ZZMReclaimMemory] [%p, %p, %lu], bad mmap result, errno: %d (%s)\n",  current_page, (void *)current_page_uncommit_begin, current_page->area_end() - current_page_uncommit_begin, errno, strerror(errno));
    }
    printf("[ZZM] [ZZMReclaimMemory] [%p, %p, %lu] finish\n", current_page, (void *)current_page_uncommit_begin, current_page->area_end() - current_page_uncommit_begin);
    current_page = current_page->next_page();
    if (current_page == NULL)
      break;
    current_page_uncommit_begin = current_page->area_start();
  }
  // free unmapping list
  // memory_allocator()->unmapper()->ZZMFreeChunksEagerly();
  zzm_is_mild_gc = false;

  PrintIsolate(isolate(), "[zzm] ZZMReclaimMemory exited\n");
}
void Heap::ZZMRestoreMemory() {
  PrintIsolate(isolate(), "[zzm] ZZMRestoreMemory called\n");
}
