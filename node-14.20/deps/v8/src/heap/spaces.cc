...
void MemoryAllocator::Unmapper::FreeQueuedChunks() {
  // <zzm>
  if (Heap::zzm_is_mild_gc) {
    // printf("[0518] MemoryAllocator::Unmapper::FreeQueuedChunks is_mild_gc is true\n");
    PerformFreeMemoryOnQueuedChunks<FreeMode::kReleasePooled>();
    return;
  }
  // printf("[0518] MemoryAllocator::Unmapper::FreeQueuedChunks is_mild_gc is false\n");
  // </zzm>
...
void MemoryAllocator::Unmapper::PerformFreeMemoryOnQueuedChunks() {
  ...
  // PrintIsolate(
  //     heap_->isolate(),
  //     "[ZZM] Unmapper::PerformFreeMemoryOnQueuedChunks: %d queued chunks, current mode is: %d, kUncommitPooled=%d  \n",
  //     NumberOfChunks(), mode, MemoryAllocator::Unmapper::FreeMode::kUncommitPooled);
  if (FLAG_trace_unmapper) {
    PrintIsolate(
        heap_->isolate(),
  ...
  while ((chunk = GetMemoryChunkSafe<kRegular>()) != nullptr) {
    ...
    if (pooled) {
      ...
      //   PrintIsolate(
      //       heap_->isolate(),
      //       "[ZZM] [%d] Unmapper::PerformFreeMemoryOnQueuedChunks: chunk: %p (null chunk?)  \n",
      //       cur_pooled_idx, chunk);
      // } else {
      //   PrintIsolate(
      //       heap_->isolate(),
      //       "[ZZM] [%d] Unmapper::PerformFreeMemoryOnQueuedChunks: chunk: %p (%ld KB)  \n",
      //       cur_pooled_idx, (void *)chunk->area_start(), chunk->area_size() / 1024);
      //   cur_pooled_idx += 1;
      // }
      //   PrintIsolate(
      //       heap_->isolate(),
      //       "[ZZM] [%d] Unmapper::PerformFreeMemoryOnQueuedChunks: pooled chunk:  %p\n", cur_pooled_idx, chunk);
...
// <zzm>
void MemoryAllocator::Unmapper::ZZMFreeChunksOneType(MemoryAllocator::Unmapper::ChunkQueueType type) {
  auto target_chunks = chunks_[type];
  const int size = MemoryChunk::kPageSize;
  for (auto iter = target_chunks.begin(); iter != target_chunks.end(); iter ++) {
    MemoryChunk *cur_chunk = *iter;
    printf("[zzm] [FreeChunksEagerly] [%p, %d] freeing\n", cur_chunk, size);
    void * ret = mmap((void *)cur_chunk, size, PROT_NONE, MAP_PRIVATE|MAP_FIXED|MAP_NORESERVE|MAP_ANONYMOUS, -1, 0);
    if (ret != cur_chunk) {
      printf("[zzm] FreePooledChunksEagerly %p failed, bad mmap result, errno: %d (%s)\n", cur_chunk, errno, strerror(errno));
    }
  }
}

void MemoryAllocator::Unmapper::ZZMFreeChunksEagerly() {
  base::MutexGuard guard(&mutex_);
  if (chunks_[kPooled].empty())
    return;
  const int size = MemoryChunk::kPageSize;
  auto pooled_chunks = chunks_[kPooled];
  for (auto iter = pooled_chunks.begin(); iter != pooled_chunks.end(); iter ++) {
    MemoryChunk *cur_chunk = *iter;
    printf("[zzm] [FreePooledChunksEagerly] [%p, %d] freeing\n", cur_chunk, size);
    void * ret = mmap((void *)cur_chunk, size, PROT_NONE, MAP_PRIVATE|MAP_FIXED|MAP_NORESERVE|MAP_ANONYMOUS, -1, 0);
    if (ret != cur_chunk) {
      printf("[zzm] FreePooledChunksEagerly %p failed, bad mmap result, errno: %d (%s)\n", cur_chunk, errno, strerror(errno));
    }
    printf("[zzm] [FreePooledChunksEagerly] [%p, %d] freed\n", cur_chunk, size);
  }
}
// </zzm>
...
void MemoryAllocator::Free(MemoryChunk* chunk) {
  // printf("[zzm][Free] freeing chunk: %p", chunk);
  switch (mode) {
    case kFull:
      PreFreeMemory(chunk);
